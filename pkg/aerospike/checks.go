...
func LatencyCheck(p topology.ProbeableEndpoint) error {
	...
	genericConfig := e.ClusterInfo.Config.genericConfig // TODO why only genericConfig is used
	keyPrefix := genericConfig.LatencyKeyPrefix
	...
	policy.TotalTimeout = genericConfig.TotalTimeout // 0 is default Client value in v7
	// Do not wait until timeout if connections cannot be open
	policy.ExitFastOnExhaustedConnectionPool = genericConfig.ExitFastOnExhaustedConnectionPool

	for range e.Client.Cluster().GetNodes() { // scale the number of latency checks to the number of nodes
		// TODO configurable set
		key, as_err := as.NewKey(e.Namespace, genericConfig.MonitoringSet, fmt.Sprintf("%s%s", keyPrefix, utils.RandomHex(20)))
		if as_err != nil {
			return as_err
		}
		val := as.BinMap{
		...
		// PUT OPERATION
		labels := []string{"put", node.GetHost().Name, e.Namespace, e.ClusterInfo.ClusterName, node.GetName()}
...
func DurabilityPrepare(p topology.ProbeableEndpoint) error {
	...
	policy.TotalTimeout = genericConfig.TotalTimeout    // 0 is default Client value in v7
	keyRange := genericConfig.DurabilityKeyTotal
	keyPrefix := genericConfig.DurabilityKeyPrefix
	...
	// allPushedFlag indicate if a probe have pushed all data once
	allPushedFlag, err := as.NewKey(e.Namespace, genericConfig.MonitoringSet, fmt.Sprintf("%s%s", keyPrefix, "all_pushed_flag"))
	if err != nil {
		return err
	}
	...
	for i := 0; i < keyRange; i++ {
		...
		key, err := as.NewKey(e.Namespace, genericConfig.MonitoringSet, keyName)
		if err != nil {
			return err
		}
...
func DurabilityCheck(p topology.ProbeableEndpoint) error {
	...
	durabilityExpectedItems.WithLabelValues(e.Namespace, e.ClusterInfo.ClusterName, e.GetName()).Set(float64(keyRange))
	durabilityFoundItems.WithLabelValues(e.Namespace, e.ClusterInfo.ClusterName, e.GetName()).Set(total_found_items)
	durabilityCorruptedItems.WithLabelValues(e.Namespace, e.ClusterInfo.ClusterName, e.GetName()).Set(total_corrupted_items)
	return nil
}
